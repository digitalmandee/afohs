...
const ProductsTrashed = ({ trashedProducts, filters }) => {
    ...
    const handleRestore = () => {
        ...
        router.post(
            ...
            {
                onSuccess: () => {
                    enqueueSnackbar('Product restored successfully!', { variant: 'success' });
                    setRestoreModalOpen(false);
                    setSelectedItem(null);
                },
                onError: () => enqueueSnackbar('Failed to restore product.', { variant: 'error' }),
            ...
            >
                ...
                <TableContainer sx={{ borderRadius: '12px', }}>
                    <Table>
                        ...
                        <TableBody>
                            {trashedProducts.data.length > 0 ? (
                                trashedProducts.data.map((item) => (
                                    <TableRow key={item.id} hover>
                                        <TableCell>{item.name}</TableCell>
                                        <TableCell>{item.menu_code || 'N/A'}</TableCell>
