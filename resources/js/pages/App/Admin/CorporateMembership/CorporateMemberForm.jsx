...
import { MembershipCardContent, handlePrintMembershipCard } from '../Membership/UserCard';
...
const LOCAL_STORAGE_KEY = 'corporateMemberFormData';

const CorporateMemberForm = ({ membershipNo, applicationNo, membercategories, familyMembers, user }) => {
    ...
    const handleDownloadCard = async (elementId, memberNo) => {
        ...
        try {
            ...
            link.href = dataUrl;
            link.download = `Corporate_Membership_Card_${memberNo || 'card'}.png`;
    ...
    const handleFinalSubmit = async () => {
        ...
        const isEditMode = !!user?.id;
        const url = isEditMode ? route('corporate-membership.update', user.id) : route('corporate-membership.store');

        await axios
            .post(url, formData2)
            .then((response) => {
                enqueueSnackbar(`Corporate Membership ${isEditMode ? 'updated' : 'created'} successfully.`, { variant: 'success' });
                localStorage.removeItem(LOCAL_STORAGE_KEY);

                if (response.data.member) {
                    if (!isEditMode) {
                        // Redirect to edit page with step 4
                        router.visit(route('corporate-membership.edit', response.data.member.id) + '?step=4');
                    } else {
    ...
    return (
        <>
            <div style={{ backgroundColor: '#f5f5f5', minHeight: '100vh', padding: '20px' }}>
                ...
                {step === 4 && <AddForm4 onNext={() => setStep(5)} onBack={() => setStep(3)} memberId={createdMember?.id || user?.id} initialData={createdMember || user} isCorporate={true} familyMembers={formsData.family_members} />}
                {step === 5 && (
                    <Box sx={{ display: 'flex', flexDirection: 'column', alignItems: 'center', mt: 4 }}>
                        ...
                        <Box sx={{ display: 'flex', gap: 2 }}>
                            <MuiButton variant="contained" startIcon={<PrintIcon />} onClick={() => handlePrintMembershipCard(createdMember || user)} sx={{ bgcolor: '#063455', '&:hover': { bgcolor: '#0c2461' } }}>
                                Print
                            </MuiButton>
                            <MuiButton variant="contained" startIcon={<DownloadIcon />} onClick={() => handleDownloadCard('main-member-card', (createdMember || user)?.membership_no)} sx={{ bgcolor: '#063455', '&:hover': { bgcolor: '#0c2461' } }}>
                                Download
                            </MuiButton>
                        </Box>
                    </Box>
