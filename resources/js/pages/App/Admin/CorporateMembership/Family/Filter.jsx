...
const CorporateFamilyFilter = () => {
    ...
    const handleResetFilters = () => {
        ...
        setFilters(reset);
        router.get(route('corporate-membership.family-members'));
    };
    ...
    const fetchMembers = async (query) => {
        setLoading(true);
        try {
            // Reusing member search but technically family members are corporate members too (sub-members)
            // Or should we search corporate members?
            // The search route searches main members.
    ...
    return (
        <Box backgroundColor="transparent" mb={3}>
            <Box sx={{ display: 'grid', gridTemplateColumns: { xs: 'repeat(1, 1fr)', sm: 'repeat(5, 1fr)' } }} gap={2} mb={2}>
                ...
                />
                <FormControlLabel control={<Checkbox checked={filters.age_over_25} onChange={(e) => handleFilterChange('age_over_25', e.target.checked)} key="age_over_25" />} label="Age over 25" />
                <TextField
                    select
                    label="Relation"
                    size="small"
                ...
                >
                    ...
                    {props.memberCategories &&
                        props.memberCategories.map((type, idx) => (
...
export default CorporateFamilyFilter;
