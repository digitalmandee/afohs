...
const PaymentHistoryReport = ({ bookings = {}, filters = {}, venues = [] }) => {
    const bookingList = bookings.data || [];

    const handlePrint = () => {
        const params = new URLSearchParams(window.location.search);
        const printUrl = route('events.reports.payment-history.print', Object.fromEntries(params));
        window.open(printUrl, '_blank');
    };

    const handleExport = () => {
        const params = new URLSearchParams(window.location.search);
        const exportUrl = route('events.reports.payment-history.export', Object.fromEntries(params));
    ...
    return (
        <AdminLayout>
            <Box sx={{ bgcolor: '#f5f5f5', minHeight: '100vh', p: 3 }}>
                ...
                {/* Filters */}
                <RoomBookingFilter
                    routeName="events.reports.payment-history"
                    showStatus={true}
                    showRoomType={false}
                    showVenues={true}
                        ...
                        <Table>
                            <TableHead>
                                <TableRow sx={{ backgroundColor: '#063455' }}>
                                    ...
                                    <TableCell sx={{ color: '#fff', fontWeight: 600 }}>Event Date</TableCell>
                                ...
                                {bookingList.length === 0 ? (
                                    ...
                                    bookingList.map((booking) => (
                                        <TableRow key={booking.id} sx={{ '&:hover': { backgroundColor: '#f5f5f5' } }}>
                                            ...
                                            <TableCell>{booking.invoice?.invoice_no || '-'}</TableCell>
                                            ...
                                            <TableCell>
                                                <Typography color={booking.total_price - (booking.invoice?.paid_amount || 0) > 0 ? 'error.main' : 'text.secondary'}>{booking.total_price - (booking.invoice?.paid_amount || 0)}</Typography>
                                            </TableCell>
                                        </TableRow>
                                    ))
                                )}
