...
const BookingReport = ({ bookings = {}, filters = {}, venues = [] }) => {
    const bookingList = bookings.data || [];

    const handlePrint = () => {
        const params = new URLSearchParams(window.location.search);
        const printUrl = route('events.reports.booking.print', Object.fromEntries(params));
        window.open(printUrl, '_blank');
    };

    const handleExport = () => {
        const params = new URLSearchParams(window.location.search);
        const exportUrl = route('events.reports.booking.export', Object.fromEntries(params));
    ...
    return (
        <AdminLayout>
            <Box sx={{ bgcolor: '#f5f5f5', minHeight: '100vh', p: 3 }}>
                ...
                {/* Filters - Booking Date enabled */}
                <RoomBookingFilter routeName="events.reports.booking" showStatus={true} showRoomType={false} showVenues={true} venues={venues} showDates={{ booking: true, checkIn: true, checkOut: false }} dateLabels={{ booking: 'Booking Date', checkIn: 'Event Date' }} />
