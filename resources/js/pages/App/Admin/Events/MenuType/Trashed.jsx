...
const Trashed = ({ eventMenuTypes, filters: initialFilters }) => {
    ...
    const handleRestore = (id) => {
        setProcessingId(id);
        router.post(
            route('event-menu-type.restore', id),
            {},
            {
                onSuccess: () => {
                    enqueueSnackbar('Menu Type restored successfully', { variant: 'success' });
                    setProcessingId(null);
                },
                onError: () => {
                    enqueueSnackbar('Failed to restore menu type', { variant: 'error' });
                    setProcessingId(null);
                },
            },
        );
        ...
        <Box sx={{ minHeight: '100vh', backgroundColor: '#f5f5f5', padding: '20px' }}>
            <Box sx={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', mb: 3 }}>
                <Box sx={{ display: 'flex', alignItems: 'center', gap: 2 }}>
                    <IconButton onClick={() => router.get(route('event-menu-type.index'))}>
                        <ArrowBack sx={{ color: '#063455' }} />
                    </IconButton>
                    <Typography sx={{ fontWeight: 700, fontSize: '30px', color: '#063455' }}>Deleted Menu Types</Typography>
                </Box>
            ...
            <TableContainer component={Paper} style={{ boxShadow: 'none', overflowX: 'auto', borderRadius: '16px' }}>
                <Table>
                    ...
                    <TableBody>
                        {eventMenuTypes.data.length > 0 ? (
                            eventMenuTypes.data.map((item) => (
                                <TableRow key={item.id} style={{ borderBottom: '1px solid #eee' }}>
                                    ...
                                    <TableCell>
                                        ...
                                        <Button
                                            ...
                                            onClick={() => {
                                                if (confirm('Are you sure you want to permanently delete this menu type? This action cannot be undone.')) {
                                                    router.delete(route('event-menu-type.force-delete', item.id), {
                                                        onSuccess: () => enqueueSnackbar('Menu Type deleted permanently', { variant: 'success' }),
                                                        onError: () => enqueueSnackbar('Failed to delete menu type', { variant: 'error' }),
                                                    });
                                                }
                                            }}
                                            sx={{ ml: 1 }}
            ...
            <Pagination className="mt-6" links={eventMenuTypes.links} />
        </Box>
    );
};
