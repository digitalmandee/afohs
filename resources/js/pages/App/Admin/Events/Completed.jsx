...
const EventsCompleted = ({ bookings, aggregates }) => {
        ...
        <>
            <LocalizationProvider dateAdapter={AdapterDayjs}>
                <PageShell
                    ...
                    subtitle="Review completed event bookings inside the current premium reporting shell with modern tables and preserved event-history actions."
                    filterSubtitle="Filter completed events by guest, venue, booking date, and event date without dropping back to the legacy table layout."
                    filterContent={<RoomBookingFilter embedded routeName="events.completed" showRoomType={false} showVenues venues={venues} showStatus={false} showDates={{ booking: true, checkIn: true, checkOut: false }} dateLabels={{ booking: 'Booking Date', checkIn: 'Event Date' }} />}
                    tableTitle="Completed Event Register"
                    tableSubtitle="Modernized completed-event table with current colors, spacing, and summary totals."
                >
                    <AdminDataTable
                        columns={[
                        ...
                        tableMinWidth={1880}
                        stickyLastColumn
                        emptyMessage="No completed event bookings found."
                        renderRow={(booking) => {
