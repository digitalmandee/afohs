...
const BankTransfer = ({ employees = [], periods = [], totals = null, filters = {} }) => {
    ...
    const handlePrint = () => {
        ...
        const printUrl = route('employees.reports.bank-transfer.print', {
            period_id: selectedPeriod,
        });
    ...
    const handleExport = () => {
        if (!selectedPeriod) return;
        window.open(`/admin/employees/reports/api/bank-transfer/export?period_id=${selectedPeriod}`, '_blank');
    };
                        ...
                        <Table>
                            <TableHead>
                                <TableRow sx={{ backgroundColor: '#063455' }}>
                                    ...
                                    <TableCell sx={{ color: '#fff', fontWeight: 600 }}>Bank Name</TableCell>
                                    <TableCell sx={{ color: '#fff', fontWeight: 600 }}>Account No</TableCell>
                                    <TableCell sx={{ color: '#fff', fontWeight: 600 }}>Branch Code</TableCell>
                                ...
                                {employees.length === 0 ? (
                                    <TableRow>
                                        <TableCell colSpan={8} align="center" sx={{ py: 4 }}>
                                            <Typography color="textSecondary">{selectedPeriod ? 'No data found for this period' : 'Please select a payroll period'}</Typography>
                                        </TableCell>
                                    </TableRow>
                                ) : (
                                    employees.map((emp, index) => (
                                        <TableRow key={index} sx={{ '&:hover': { backgroundColor: '#f5f5f5' } }}>
                                            ...
                                            <TableCell sx={{ fontWeight: 500 }}>{emp.employee_name}</TableCell>
