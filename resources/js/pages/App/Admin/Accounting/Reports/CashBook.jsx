...
export default function CashBook({ rows, summary = {}, filters = {}, bookLabel = 'Cash Book', error = null, accounts = [], paymentAccounts = [], tenants = [], sourceOptions = [] }) {
    ...
    const apply = React.useCallback((next) => {
        router.get(route('accounting.reports.cash-book'), sanitizeFilters(next), {
            preserveState: true,
            preserveScroll: true,
            replace: true,
        });
    ...
    return (
        <AppPage
            ...
            title={bookLabel}
            subtitle="Running balance view for cash-side journal movements."
            actions={[
                <Button key="pdf" variant="outlined" onClick={() => downloadReportPdf('accounting.reports.cash-book.pdf', localFilters)}>Download PDF</Button>,
                <Button key="csv" variant="outlined" onClick={() => downloadReportCsv('accounting.reports.cash-book', localFilters)}>Export CSV</Button>,
                <Button key="print" variant="outlined" onClick={() => openReportPrint('accounting.reports.cash-book.print', localFilters)}>Print</Button>,
            ]}
        >
            ...
            <SurfaceCard lowChrome>
                <AdminDataTable
                    ...
                    pagination={rows}
                    tableMinWidth={1180}
                    emptyMessage="No cash book records found."
                    renderRow={(row) => (
