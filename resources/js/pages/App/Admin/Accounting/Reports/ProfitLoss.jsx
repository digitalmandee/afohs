...
export default function ProfitLoss({ income, expense, summary, filters }) {
  ...
  return (
    <Box>
      <Typography variant="h4" sx={{ mb: 2, color: 'primary.main', fontWeight: 700, '@media print': { display: 'none' } }}>Profit & Loss</Typography>

      <Box sx={{ display: 'none', '@media print': { display: 'block', mb: 2 } }}>
        <Typography variant="h5" sx={{ fontWeight: 700, color: 'primary.main' }}>Profit & Loss</Typography>
      ...
      <Grid container spacing={2} sx={{ mb: 3 }}>
        <Grid item xs={12} md={4}><StatCard label="Income" value={Number(summary?.income_total || 0).toFixed(2)} tone="muted" /></Grid>
        <Grid item xs={12} md={4}><StatCard label="Expense" value={Number(summary?.expense_total || 0).toFixed(2)} /></Grid>
        <Grid item xs={12} md={4}><StatCard label="Net Profit" value={Number(summary?.net_profit || 0).toFixed(2)} /></Grid>
      </Grid>
          ...
          <form onSubmit={applyFilters}>
            <Grid container spacing={2}>
              ...
              <Grid item xs={12} md={1}>
                <Button type="button" variant="outlined" sx={{ mt: { md: 1 } }} onClick={() => router.get(route('accounting.reports.profit-loss'))}>
                  Reset
                </Button>
              </Grid>
            </Grid>
      ...
      <Box sx={{ display: 'none', '@media print': { display: 'block', mt: 1 } }}>
        <Typography variant="caption" color="text.secondary">
          Totals: Income {Number(summary?.income_total || 0).toFixed(2)} | Expense {Number(summary?.expense_total || 0).toFixed(2)} | Net {Number(summary?.net_profit || 0).toFixed(2)}
        </Typography>
      </Box>
    </Box>
  );
