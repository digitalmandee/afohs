...
export default function PayablesAging({ rows, summary, filters }) {
    ...
    const submitFilters = React.useCallback((nextFilters) => {
        ...
        router.get(route('accounting.reports.payables-aging'), payload, {
            preserveState: true,
            preserveScroll: true,
            replace: true,
        });
    ...
    const columns = [
        { key: 'bill', label: 'Bill', minWidth: 150 },
    ...
    return (
        <AppPage
            ...
            subtitle="Track vendor liability exposure by due-date bucket with the same live filters and premium register styling as the rest of accounting."
            actions={[
                <Button key="pdf" variant="outlined" onClick={() => downloadReportPdf('accounting.reports.payables-aging.pdf', sanitizeFilters(localFilters))}>Download PDF</Button>,
                <Button key="print" variant="outlined" onClick={() => openReportPrint('accounting.reports.payables-aging.print', sanitizeFilters(localFilters))}>Print</Button>,
            ]}
        >
            <Grid container spacing={2.25}>
                <Grid item xs={12} md={3}><StatCard label="Open Bills" value={formatReportCount(summary?.records)} accent /></Grid>
            ...
            <SurfaceCard
                title="Aging Buckets"
                subtitle="Live distribution of open payables across aging bands."
                actions={<Button variant="outlined" onClick={() => openReportPrint('accounting.reports.payables-aging.print', sanitizeFilters(localFilters))}>Print</Button>}
            >
            ...
            <SurfaceCard title="Payables Aging Register" subtitle="Branded aging table with shared pagination and printable summary support.">
                <AdminDataTable
                    columns={columns}
                    rows={data}
                    loading={loading}
