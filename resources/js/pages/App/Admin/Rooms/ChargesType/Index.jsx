...
const RoomChargesType = ({ roomChargesData }) => {
    ...
    const [roomCharges, setRoomCharges] = useState(roomChargesData || []);
    ...
    const handleDelete = async () => {
        ...
        try {
            await axios.delete(route('room-charges-type.destroy', roomToDelete.id), {
                headers: { 'X-CSRF-TOKEN': csrfToken },
            });
            setRoomCharges((prev) => prev.filter((type) => type.id !== roomToDelete.id));
            enqueueSnackbar('Room Charge Type deleted successfully.', { variant: 'success' });
        } catch (error) {
    ...
    return (
        <>
            ...
            >
                ...
                <Typography style={{ color: '#063455', fontSize: '15px', fontWeight: '600' }}>Configure different types of room-related charges applied to guests</Typography>
                    ...
                    <Table>
                        <TableHead>
                            <TableRow style={{ backgroundColor: '#063455', height: '30px' }}>
                                ...
                                <TableCell sx={{ color: '#fff', fontSize: '14px', fontWeight: 600 }}>Room Charge Type</TableCell>
            ...
            <AddRoomModal open={modalOpen} handleClose={() => setModalOpen(false)} roomChargesType={editingRoom} onSuccess={handleSuccess} />

            {/* Delete Confirmation Dialog */}
            <Dialog open={deleteDialogOpen} onClose={cancelDelete} aria-labelledby="delete-dialog-title">
                <DialogTitle id="delete-dialog-title">Delete Room Charge Type</DialogTitle>
                <DialogContent>
                    <DialogContentText>
...
export default RoomChargesType;
