...
const Trashed = ({ roomCharges, filters: initialFilters }) => {
    ...
    const handleSearch = () => {
        router.get(
            route('room-charges-type.trashed'),
            { search },
            {
                preserveState: true,
                preserveScroll: true,
    ...
    const handleRestore = (id) => {
        setProcessingId(id);
        router.post(
            route('room-charges-type.restore', id),
            {},
            {
                onSuccess: () => {
                    enqueueSnackbar('Room Charge Type restored successfully', { variant: 'success' });
                    setProcessingId(null);
                },
                onError: () => {
                    enqueueSnackbar('Failed to restore room charge type', { variant: 'error' });
                    setProcessingId(null);
                },
            },
        );
        ...
        <Box sx={{ minHeight: '100vh', backgroundColor: '#f5f5f5', padding: '20px' }}>
            <Box sx={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', mb: 3 }}>
                <Box sx={{ display: 'flex', alignItems: 'center', gap: 2 }}>
                    <IconButton onClick={() => router.get(route('room-charges-type.index'))}>
                        <ArrowBack sx={{ color: '#063455' }} />
                    </IconButton>
                    <Typography sx={{ fontWeight: 700, fontSize: '30px', color: '#063455' }}>Deleted Room Charges Types</Typography>
                </Box>
            ...
            <TableContainer component={Paper} style={{ boxShadow: 'none', overflowX: 'auto', borderRadius: '16px' }}>
                <Table>
                    ...
                    <TableBody>
                        {roomCharges.data.length > 0 ? (
                            roomCharges.data.map((item) => (
                                <TableRow key={item.id} style={{ borderBottom: '1px solid #eee' }}>
                                    ...
                                    <TableCell>
                                        ...
                                        <Button
                                            ...
                                            onClick={() => {
                                                if (confirm('Are you sure you want to permanently delete this room charge type? This action cannot be undone.')) {
                                                    router.delete(route('room-charges-type.force-delete', item.id), {
                                                        onSuccess: () => enqueueSnackbar('Room Charge Type deleted permanently', { variant: 'success' }),
                                                        onError: () => enqueueSnackbar('Failed to delete room charge type', { variant: 'error' }),
                                                    });
                                                }
                                            }}
                                            sx={{ ml: 1 }}
                            ...
                            <TableRow>
                                <TableCell colSpan={5} align="center" sx={{ py: 3 }}>
                                    No deleted room charge types found.
                                </TableCell>
                            </TableRow>
                        )}
                    </TableBody>
            ...
            <Pagination className="mt-6" links={roomCharges.links} />
        </Box>
    );
};
