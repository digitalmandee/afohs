...
const Trashed = ({ roomCategories, filters: initialFilters }) => {
    ...
    const handleRestore = (id) => {
        setProcessingId(id);
        router.post(
            route('room-categories.restore', id),
            {},
            {
                onSuccess: () => {
                    enqueueSnackbar('Room Category restored successfully', { variant: 'success' });
                    setProcessingId(null);
                },
                onError: () => {
                    enqueueSnackbar('Failed to restore room category', { variant: 'error' });
                    setProcessingId(null);
                },
            },
        );
        ...
        <Box sx={{ minHeight: '100vh', backgroundColor: '#f5f5f5', padding: '20px' }}>
            <Box sx={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', mb: 3 }}>
                <Box sx={{ display: 'flex', alignItems: 'center', gap: 2 }}>
                    <IconButton onClick={() => router.get(route('room-categories.index'))}>
                        <ArrowBack sx={{ color: '#063455' }} />
                    </IconButton>
                    <Typography sx={{ fontWeight: 700, fontSize: '30px', color: '#063455' }}>Deleted Room Categories</Typography>
                </Box>
            ...
            <TableContainer component={Paper} style={{ boxShadow: 'none', overflowX: 'auto', borderRadius: '16px' }}>
                <Table>
                    ...
                    <TableBody>
                        {roomCategories.data.length > 0 ? (
                            roomCategories.data.map((item) => (
                                <TableRow key={item.id} style={{ borderBottom: '1px solid #eee' }}>
                                    ...
                                    <TableCell>
                                        ...
                                        <Button
                                            ...
                                            onClick={() => {
                                                if (confirm('Are you sure you want to permanently delete this room category? This action cannot be undone.')) {
                                                    router.delete(route('room-categories.force-delete', item.id), {
                                                        onSuccess: () => enqueueSnackbar('Room Category deleted permanently', { variant: 'success' }),
                                                        onError: () => enqueueSnackbar('Failed to delete room category', { variant: 'error' }),
                                                    });
                                                }
                                            }}
                                            sx={{ ml: 1 }}
                            ...
                            <TableRow>
                                <TableCell colSpan={4} align="center" sx={{ py: 3 }}>
                                    No deleted room categories found.
                                </TableCell>
                            </TableRow>
                        )}
                    </TableBody>
            ...
            <Pagination className="mt-6" links={roomCategories.links} />
        </Box>
    );
};
